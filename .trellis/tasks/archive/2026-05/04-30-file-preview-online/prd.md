...
* 预览 UI 使用全屏覆盖页，由 `FileListState.previewFileName` 和 `PreviewState` 驱动，系统返回/顶部返回关闭预览。
* 图片预览使用 Coil 从 SMB 读取到的内存字节解码，避免在 UI 层直接访问远端网络。
* 实现过程中已扩展支持视频预览：SMB 流先写入本地缓存文件，再交给 Media3/ExoPlayer 播放；缓存生命周期由 ViewModel 负责。

## Requirements (evolving)

* 文件菜单新增「预览」选项，仅对可预览类型（图片 / 文本 / 视频）显示
* 图片预览：流式从 SMB 读取 → 显示，支持双指缩放、返回关闭
* 文本预览：流式从 SMB 读取（最多 1 MB）→ 可滚动文本，支持返回关闭
* 加载中显示进度指示器；加载失败显示错误信息
* 支持浅色/深色主题

## Acceptance Criteria (evolving)

* [x] 图片文件（jpg/png/gif/webp）在菜单中显示「预览」选项
* [x] 点击预览后能看到图片内容（无需下载）
* [x] 文本文件（txt/log/md/xml/json）在菜单中显示「预览」选项
* [x] 点击预览后能看到文本内容，可滚动
* [x] 文件 > 1 MB 时，文本预览提示截断警告
* [x] 加载失败时显示错误信息，可关闭预览
* [x] 单测覆盖文件类型判断逻辑（已有 FileTypeHelper）
* [x] UI 兼容浅色/深色主题
