...
class SMBFileRepository @Inject constructor(
    ...
    fun getFileInputStream(filePath: String): java.io.InputStream {
        ...
        val lease = connectionManager.acquireDiskShare()

        try {
            val normalizedPath = normalizePath(filePath)
            Log.d(TAG, "规范化路径: $normalizedPath")
            val file: File = lease.diskShare.openFile(
                normalizedPath,
                setOf(AccessMask.GENERIC_READ),
                null,
                setOf(
            ...
            return file.asClosingInputStream(lease)
        } catch (e: Exception) {
            lease.close()
            Log.e(TAG, "打开文件失败", e)
    ...
    private fun File.asClosingInputStream(lease: SMBConnectionManager.DiskShareLease): InputStream {
        val input = try {
            inputStream
        } catch (e: Exception) {
