...
import com.qi.smb_share_android.data.local.TransferDatabase
import com.qi.smb_share_android.data.local.TransferTaskDao
import com.qi.smb_share_android.data.local.toEntity
import com.qi.smb_share_android.data.local.toModel
...
import com.qi.smb_share_android.util.toJsonString
...
class TransferRepository(private val context: Context) {
    ...
    private val database: TransferDatabase = TransferDatabase.getInstance(context)
    private val taskDao: TransferTaskDao = database.transferTaskDao()
    ...
    suspend fun pauseTransfer(taskId: String) {
        ...
        // 通知传输服务暂停任务
        notifyTransferService(com.qi.smb_share_android.service.TransferService.ACTION_PAUSE_TRANSFER, taskId)
    }
    ...
    suspend fun resumeTransfer(taskId: String) {
        ...
        // 通知传输服务恢复任务
        notifyTransferService(com.qi.smb_share_android.service.TransferService.ACTION_RESUME_TRANSFER, taskId)
    }
    ...
    suspend fun cancelTransfer(taskId: String) {
        ...
        // 通知传输服务取消任务
        notifyTransferService(com.qi.smb_share_android.service.TransferService.ACTION_CANCEL_TRANSFER, taskId)
    }
    ...
    private fun notifyTransferService(action: String, taskId: String) {
        val intent = Intent(context, com.qi.smb_share_android.service.TransferService::class.java).apply {
            this.action = action
            putExtra(com.qi.smb_share_android.service.TransferService.EXTRA_TASK_ID, taskId)
        }
    ...
    private fun startTransferService(taskId: String, config: SMBConfig) {
        val intent = Intent(context, com.qi.smb_share_android.service.TransferService::class.java).apply {
            action = com.qi.smb_share_android.service.TransferService.ACTION_START_TRANSFER
            putExtra(com.qi.smb_share_android.service.TransferService.EXTRA_TASK_ID, taskId)
            putExtra(com.qi.smb_share_android.service.TransferService.EXTRA_CONFIG, config.toJsonString())
        }
